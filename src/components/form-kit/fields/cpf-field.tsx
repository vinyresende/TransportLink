...
export default function CPFField({ ref, name, label, initialValue, placeholder = "", required }: Props) {
    ...
    const validate = (): boolean => {
        if (required && !inputRef.current?.value) {
            ...
            return false
        } else if (inputRef.current?.value && inputRef.current.value.length < 14) {
            setError('Digite um CPF completo!')
            return false
        }
    ...
    const setValue = (val: string): void => {
        if (inputRef?.current) {
            inputRef.current.value = val.replace(/\D/g, '').replace(/(\d{3})(\d{3})(\d{3})(\d{2})/, "$1.$2.$3-$4")
        }
    }
